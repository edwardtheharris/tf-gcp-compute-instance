...
resource "google_compute_instance" "docker" {
  ...
  provisioner "local-exec" {
    command     = "sh ${path.module}/_scripts/wait-for-ssh.sh ${google_compute_instance.docker.network_interface[0].access_config[0].nat_ip} ${var.local_keys.user} ${base64decode(var.local_keys.gpg)}"
  }
